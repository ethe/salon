...
class SalonAgent(BaseAgent):
    ...
    def _parse_result(self, result_file: Path) -> AgentResult:
        ...
        total_input_tokens = sum(int(guest.get("input_tokens", 0)) for guest in guests.values())
        total_output_tokens = sum(int(guest.get("output_tokens", 0)) for guest in guests.values())
        return AgentResult(
            total_input_tokens=total_input_tokens,
            total_output_tokens=total_output_tokens,
        ...
        for runtime_file in sorted(guests_dir.glob("*.json")):
            ...
            try:
                if guest_type == "claude":
                    log_path = self._resolve_claude_session_log_path(session_id, runtime_data.get("workspaceDir"))
                    if not log_path:
    ...
    def _resolve_codex_session_log_path(self, session_id: str, started_at: float | None) -> Path | None:
